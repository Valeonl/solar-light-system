...
ножек на самом микроконтроллере (5 минут), а дальше всё вставляется в плату-переходник. Дальше я или телефон пишем на него текст — и он появляется на экране.</p>
...
<table>
  <tr><th>№</th><th>Что с чем</th><th>Чем</th><th>Куда именно</th></tr>
  <tr><td>—</td><td>Ножки микроконтроллера</td><td><b>пайка</b> (один раз)</td>
      <td>гребёнки лежат в комплекте отдельно: припаиваются к плате, после чего плата
          вставляется ножками в гнёзда переходника</td></tr>
  <tr><td>1</td><td>Аккумулятор → переходник</td><td>разъём отсека в гнездо PH2.0</td>
      <td>красный — плюс, чёрный — минус; паять не надо, разъём просто защёлкивается</td></tr>
  <tr><td>3</td><td>Дисплей → переходник</td><td>провод «мама-мама»</td><td>VCC → 3V3</td></tr>
  <tr><td>4</td><td>Дисплей → переходник</td><td>провод «мама-мама»</td><td>GND → GND</td></tr>
  <tr><td>5</td><td>Дисплей → переходник</td><td>провод «мама-мама»</td><td>SDA → GPIO8 (данные)</td></tr>
  <tr><td>6</td><td>Дисплей → переходник</td><td>провод «мама-мама»</td><td>SCL → GPIO9 (такт)</td></tr>
  <tr><td>7</td><td>Телефон/ноутбук → устройство</td><td>Bluetooth</td>
      <td>по воздуху, 5–10 м: только текст. Прошивка — кабелем USB, один раз</td></tr>
  <tr><td>—</td><td>Зарядка аккумулятора</td><td>TP4056 либо внешняя зарядка</td>
      <td>у TP4056 площадки B+ / B− рассчитаны на пайку: это единственное место, где паять
          действительно уместно. Либо аккумулятор вынимается и заряжается в обычной зарядке</td></tr>
</table>

<figure>
  {{SVG_WIRING}}
  <figcaption>Подробная схема: сверху — основной вариант с платой-переходником, снизу —
  вариант без переходника на макетной плате. Номера на проводах совпадают с таблицей.</figcaption>
</figure>
...
<div class="box">
  ...
  Макетная плата ({money(OPT['brd']['price'])} ₽) нужна только если собирать без переходника —
  тогда в неё вставляются и плата, и дисплей, и все провода.
</div>
<div class="box">
  <b>Что с разъёмом батарейного отсека.</b> У отсека на проводах — маленький белый
  двухконтактный разъём. На переходнике точно такое же гнездо: скорее всего они совпадут и
  соединятся щелчком. Если шаг не совпадёт — между ними ставится винтовая клеммная колодка
  ({money(119)} ₽) или короткий переходной кабель: это и есть её роль в смете, а не «странный
  выход» — она не обязательна, но снимает риск несовпадения разъёмов.
</div>
