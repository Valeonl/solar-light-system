...
<div class="box">
  ...
  <b>Что докупить:</b> резисторы <b>300 кОм, 1 Вт, 10 шт — 121 ₽</b> (★5.0, 4 561 отзыв,
  доставка завтра). Нужны всего два одинаковых, остальные останутся в запасе. Подойдут и любые
  два одинаковых номиналом от 10 кОм до 300 кОм: если номинал другой, в прошивке меняется одна
  строка — <code>BAT_DIVIDER</code> (при двух одинаковых это по-прежнему 2,0). Делитель
  обязателен: подать 4,2 В прямо на вход, рассчитанный на 3,3 В, нельзя — вывод можно испортить,
  одним кодом это не обходится.<br><br>
  Честно про точность: литий разряжается неравномерно, а под нагрузкой напряжение проседает,
  поэтому процент — «примерно», зато порог «пора на зарядку» видно надёжно. У резисторов на
  300 кОм показания могут слегка «плясать» — тогда либо взять пару на 13 кОм (источник
  низкоомнее, точнее), либо добавить конденсатор 0,1 мкФ между <code>GPIO4</code> и <code>GND</code>.
</div>
<div class="box warn">
