...
def card(i: dict) -> str:
    ...
    return (
        ...
        f'<span class="days">доставка {i["days"]} дн.</span></div>\n'
...
Bluetooth. Собирается без пайки: микроконтроллер, экран и питание соединяются шлейфами и
винтовыми клеммами. Дальше я или телефон пишем на него текст — и он появляется на экране.</p>

<div class="kpis">
  <div class="kpi"><b>{money(TOTAL)} ₽</b><span>вся сборка: {len(BUILD)} позиций, доставка до 7 дней</span></div>
  <div class="kpi"><b>★4.7–4.9</b><span>у всех товаров есть отзывы (168–4 165 штук)</span></div>
  <div class="kpi"><b>≈{HOURS:.0f} ч</b><span>работа от одного заряда ({CELL_MAH:.0f} мА·ч, ток ≈{ESP_MA:.0f} мА)</span></div>
  <div class="kpi"><b>0</b><span>паяных соединений</span></div>
</div>
...
<h2>Как это соединяется (у платы нет ножек)</h2>
<table>
  <tr><th>Что с чем</th><th>Чем соединяется</th><th>Куда именно</th></tr>
  <tr><td>ESP32-C3 → плата расширения</td><td>вставляется в разъёмы расширения</td>
      <td>своих ножек у микроплаты нет вообще, поэтому без переходной платы подключить нечего</td></tr>
  <tr><td>Экран → плата расширения</td><td>4 провода <b>«папа-мама»</b></td>
      <td>у экрана ножки-«папа», поэтому со стороны экрана нужна «мама», а в разъёмы платы
          идёт «папа»: VCC → 3V3, GND → GND, SDA → GPIO8, SCL → GPIO9</td></tr>
  <tr><td>Батарейный отсек → плата зарядки</td><td>винтовая клеммная колодка</td>
      <td>провода отсека и ответные провода зажимаются винтами: плюс → B+, минус → B−</td></tr>
  <tr><td>Плата зарядки → плата расширения</td><td>2 провода «папа-мама»</td>
      <td>OUT+ → 5V, OUT− → GND платы расширения</td></tr>
</table>
<div class="box">
  <b>Почему «папа-мама», а не «мама-мама».</b> Шлейф «мама-мама» подходит, только если у обеих
  сторон есть ножки. У ESP32-C3 их нет совсем, у платы зарядки — только отверстия под пайку.
  Поэтому берём «папа-мама»: он садится и на ножки экрана, и в разъёмы расширения, а голые
  провода отсека соединяются через винтовую колодку.
</div>

<h2>Схема</h2>
<figure>
  {{SVG_SCHEMA}}
  <figcaption>Питание: отсек → плата зарядки → плата расширения. Экран: четыре провода «папа-мама».</figcaption>
</figure>
...
<ol class="steps">
  <li>Вставь аккумулятор в батарейный отсек.</li>
  <li>Соедини отсек с платой зарядки через винтовую колодку: плюс → B+, минус → B−.
      Полярность проверь дважды — перепутанная портит плату.</li>
  <li>Выход зарядки (OUT+ и OUT−) двумя проводами «папа-мама» подключи к 5V и GND платы расширения.</li>
  <li>Вставь ESP32-C3 в плату расширения.</li>
  <li>Четырьмя проводами «папа-мама» подключи экран: VCC → 3V3, GND → GND, SDA → GPIO8, SCL → GPIO9.
      Если экран молчит — поменяй местами SDA и SCL.</li>
...
<footer>
  Цены, наличие и отзывы — Ozon, {PRICE_DATE}, фильтр «доставка до 7 дней». Товары без отзывов
