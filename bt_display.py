...
ножек на самом микроконтроллере (5 минут). Дальше я или телефон пишем на него текст — и он появляется на экране.</p>
...
<h2>Как это соединяется</h2>
<table>
  <tr><th>Что с чем</th><th>Чем соединяется</th><th>Куда именно</th></tr>
  <tr><td>Ножки микроконтроллера</td><td><b>пайка</b> — единственная в сборке</td>
      <td>гребёнки лежат в комплекте отдельно и не распаяны: припаиваются к плате, после чего
          плата вставляется в макетную плату</td></tr>
  <tr><td>Экран → макетная плата</td><td>4 провода <b>«мама-мама»</b></td>
      <td>у экрана ножки уже распаяны продавцом; «мама» надевается и на ножки экрана, и на ножки
          платы: VCC → 3V3, GND → GND, SDA → GPIO8, SCL → GPIO9</td></tr>
  <tr><td>Батарейный отсек → плата зарядки</td><td>винтовая клеммная колодка</td>
      <td>четыре провода зажимаются винтами: плюс отсека → B+, минус отсека → B−,
          выход OUT+ и OUT− → в макетную плату</td></tr>
  <tr><td>Плата зарядки → макетная плата</td><td>2 провода «мама-мама»</td>
      <td>OUT+ → 5V, OUT− → GND</td></tr>
</table>
<div class="box">
  <b>Почему «мама-мама», а не «папа-мама».</b> «Мама» нужна там, где есть штырёк. После пайки
...
<div class="box">
  ...
  есть <b>макетная плата</b>: плата, экран и провода вставляются в неё, и вся конструкция
  становится жёсткой, как будто всё припаяно. Это стандартный способ у радиолюбителей.<br><br>
